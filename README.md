# Carte Nucleo pour la SN

Repo pour les projets SN sur carte Nucleo à l'IUT de Cachan

## Règles à respecter

* Avant de démarrer un __projet Arduino__, vous devez avoir un __clone local__ de votre
repo.
...
taper la touche _Entrée_
```powershell
choco install -y ninja cmake --installargs 'ADD_CMAKE_TO_PATH=System' mingw gcc-arm-embedded git vscode 7zip python
```

* Taper la 4ème commande suivante dans la fenêtre de commande powershell puis
taper la touche _Entrée_
```powershell
pip install Jinja2 graphviz
```
...
```
* Taper la 5ème commande suivante dans la fenêtre de commande puis
taper la touche _Entrée_
```bash
pip install Jinja2 graphviz
```
...
```bash
sudo apt update
sudo apt -y install openocd cmake ninja-build git python3 python3-pip
pip install Jinja2 graphviz
```
